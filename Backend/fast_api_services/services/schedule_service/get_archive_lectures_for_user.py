...
def filter_archive_data(user_id):
    ...
    # Фильтрация данных на основе пользовательских исключений и прошедшей даты
        ...
        if sheet_name not in global_exclude and sheet_name not in users_exclude:
            for entry in entries:
                if len(entry['data']) > 2 and ', ' in entry['data'][2]:
                    ...
                    entry_date = datetime.strptime(entry['data'][2].split(', ')[1], '%d.%m.%y')
                    if entry_date < today:
...
def get_archive_lectures_main(user_id):
    data = filter_archive_data(user_id)
    if data:
...
if __name__ == "__main__":
    user_id = 1
    data = get_archive_lectures_main(user_id)
    print(data)
