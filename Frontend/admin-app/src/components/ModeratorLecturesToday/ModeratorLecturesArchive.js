...
const [sortOrder, setSortOrder] = useState('desc'); // Для сортировки по дате
const [selectedSheetNames, setSelectedSheetNames] = useState([]); // Для выбранных sheetNames
const [uniqueSheetNames, setUniqueSheetNames] = useState([]); // Для всех уникальных sheetNames
...
const filteredLectures = lectures
    .filter((lecture) =>
        selectedSheetNames.length === 0 || selectedSheetNames.includes(lecture.sheet_name)
    )
    .sort((a, b) => {
...
const currentLectures = Object.keys(groupedLectures).slice(indexOfFirstItem, indexOfLastItem);




const handleNextPage = () => {
    if (currentPage < Math.ceil(Object.keys(groupedLectures).length / itemsPerPage)) {
...
}, [sortOrder]);  // Добавляем зависимость от sortOrder
        ...
        <Box sx={{display: 'flex', minHeight: '100vh'}}>
            ...
            <AppBar position="fixed" sx={{display: {xs: 'block', md: 'none'}, backgroundColor: '#2196f3'}}>
                <Toolbar sx={{display: 'flex', justifyContent: 'space-between'}}>
                    ...
                    <Typography variant="h6">Архив вебинаров</Typography>
                    <IconButton
                        edge="end"
                        color="inherit"
                        aria-label="close"
...
<Box sx={{display: 'flex', flexDirection: 'column', marginBottom: '84px', marginTop: '10px'}}>

                    {filteredLectures.length > 0 ? (
    filteredLectures.slice(indexOfFirstItem, indexOfLastItem).map((lecture, index) => (
      <StyledCard key={lecture.id} onClick={() => handleCardClick(lecture, false)}>
        <CardHeader
        ...
        <CardContent>
          ...
          {!lecture.conferenceData?.meeting_details[0].inherited && (
            <>
        ...
        <VideoIconButton
          isColored={Boolean(lecture.conferenceData)}
          disabled={!Boolean(lecture.conferenceData)}
          onClick={(event) => {
            event.stopPropagation();
            handleOverlayOpen(lecture.id);
          }}
        >
          <VideoLibraryIcon />
        </VideoIconButton>
                        ...
                        <Typography>Загрузка архива.</Typography>
                    )}
                    ...
                    <Box display="flex" justifyContent="space-between" alignItems="center" mt={2}>
                        ...
                        <Typography>
                            Страница {currentPage} из {Math.ceil(Object.keys(groupLecturesByConference(lectures)).length / itemsPerPage)}
                        </Typography>
                        <Button onClick={handleNextPage} disabled={currentPage === Math.ceil(Object.keys(groupLecturesByConference(lectures)).length / itemsPerPage)}>
                            Следующая
                        </Button>
